...
def test_tool_card_sections_use_independent_collapsed_transitions():
    assert ".tool-card-section-body{max-height:0;opacity:0;overflow:hidden;" in COMPACT_CSS
    assert re.search(
        r"\.tool-card-section\.open\s+\.tool-card-section-body\s*\{[^}]*max-height:\s*320px;[^}]*opacity:\s*1;",
        STYLE_CSS,
    )
    assert ".tool-card-section-toggle-icon{" in COMPACT_CSS
...
def test_tool_card_toggle_uses_same_chevron_icon_markup_as_thinking_card():
    ...
    assert "class=\"tool-card-section-toggle\"" in UI_JS
    assert "_toggleToolCardSection(this)" in UI_JS
